...
$(document).ready(function() {
    $(document).on('click', '.create, .delete, .update', function() {
    	...
    	var cabinet = $(this).closest('tr').find('input.cabinet').val();
    	var date = $(this).closest('tr').find('input.date').val();
    	var time = $(this).closest('tr').find('input.time').val();
            $.ajax({
            type: 'POST',
            url: 'adminTables?dao=Procedures',
            data: {id: id, cabinet: cabinet, action: $(this).attr('class'), 
            	date: date, time: time},
...
<body>
	<table>
		<tr>
			...
			<td><input type="text" value="Cabinet"   class="cabinet"/></td>
			<td><input type="text" value="Date" class="date"/></td>
			<td><input type="text" value="Time" class="time" /></td>
		</tr>
		<c:forEach var="arg" items="${args}">
			<tr>
				<td><input type="text"   value="${arg.id}"             class="id" /></td>
				<td><input type="text"   value="${arg.cabinet}" 	   class="cabinet" /></td>
				<td><input type="text"   value="${arg.date}"       	   class="date" /></td>
				<td><input type="text"   value="${arg.time}"           class="time" /></td>
		...
		<tr>
			...
			<td><input type="text" value="" class="cabinet" /></td>
			<td><input type="text" value="" class="date" /></td>
			<td><input type="text" value="" class="time" /></td>
